...
def add_applet(device, content: bytes):
    ...
    if applet_type == AppletType.REGULAR:
        ...
        logger.info(f'Installing applet {header["name"]}')
        ...
        logger.info('Initialization for writing the applet')
        ...
        logger.info('Initialized writing the applet')

        _write_applet_content(device, content)

        logger.info('Finalizing writing the applet')
        ...
        logger.info('Finalized writing the applet')


def _write_applet_content(device, content):
    logger.info('Started writing applet content')

    remaining = len(content)
    offset = 0
    while remaining > 0:
        blocksize = min(0x400, remaining)
        block = content[offset:offset + blocksize]
        ...
        device.write(block, timeout=(blocksize // 5 + 300))
    ...
    logger.info('Completed writing applet content')
...
def read_applet(device, applet_id):
    ...
    logger.debug('applet_id=%s size=%s')
